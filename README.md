...
jvpd (jvp-daemon) 是 JVP 的守护进程，负责物理机上管理虚拟机、上报虚拟机的状态等。需要在每台物理机上安装。

jvpm (jvp-manager) 是 JVP 的核心管理工具，对外提供 api 服务、虚拟机的调度管理、下发任务给 jvp-daemon 执行。
可以在任何一个节点部署，默认部署一台即可，为了高可用可以在多个节点部署，
他们会选举出一个 leader 对外提供服务，如果 leader 宕机，会自动选举出下一个节点作为 leader。

jvpa (jvp-admin) 是 JVP 的管理工具，提供一个 Web 界面，可以方便的管理虚拟机。
