...
export default function InstanceDetailPage() {
  ...
  const rawNodeName = params.node_name as string;
  const rawInstanceId = params.id as string;
  const { nodeName, instanceId } = useMemo(() => {
    // When statically exported, params can be placeholder values.
    const segments = pathname.split("/").filter(Boolean);
    const urlNode = segments[1];
    const urlInstance = segments[2];
    const isPlaceholderNode = rawNodeName === "placeholder-node";
    const isPlaceholderInstance = rawInstanceId === "placeholder-id";
    return {
      nodeName: isPlaceholderNode && urlNode ? urlNode : rawNodeName,
      instanceId: isPlaceholderInstance && urlInstance ? urlInstance : rawInstanceId,
    };
  }, [pathname, rawInstanceId, rawNodeName]);
